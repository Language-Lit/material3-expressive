...
- Compose exposes separate Button, FilledTonalButton, ElevatedButton,
  OutlinedButton, and TextButton composables. React v1 uses one named Button
...
- Compose consumers select size, width, and square shape through modifier,
  padding, and shape objects. React v1 exposes bounded size/width/shape props and
  normal class/style layout escape hatches; arbitrary shape objects are not
  accepted.
- Compose places minimum-target expansion outside the measured visual. Web v1
