...
- `primary`-tinted shape (`ActiveIndicatorColor`). Only the uncontained
  ...
  (a colored-container variant) is not, since `V1_SPEC.md` §9 names only
  `LoadingIndicator`.
...
- **Spring easing reproduced via CSS `linear()`, not pre-sampled `d`
  ...
  technique `src/v1/tokens/css.ts` already applies to the shared
...
- Reduced motion freezes indeterminate mode on a static shape (the first
  ...
  happened to reach, matching every other v1 progress component's
