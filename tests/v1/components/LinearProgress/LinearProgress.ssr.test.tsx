...
describe('LinearProgress server rendering', () => {
  it('renders deterministic markup with no inline styles injected as <style> tags', () => {
    const render = () => renderToString(<LinearProgress aria-label="Download" value={0.4} />)
    const first = render()
  ...
  it('renders deterministic indeterminate markup with no aria-valuenow', () => {
    const render = () => renderToString(<LinearProgress aria-label="Loading" />)
    const first = render()
    expect(first).toBe(render())
  ...
  it('hydrates with zero markup delta — this component is a pure function of props with no client-only measurement effect', async () => {
    const tree = <LinearProgress aria-label="Download" value={0.4} />
