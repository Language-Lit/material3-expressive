---
name: run-v1-playground
description: Build, run, and drive the v1 (Material 3 Expressive) component playground at playground/v1. Use when asked to run the v1 playground, screenshot a v1 component, verify a new component visually, check hover/focus/state-layer/motion behavior, or interact with the running app for any T04-T29 v1 task.
---

`playground/v1` is not a runnable dev app by itself — it imports
`@language-lit/material3-expressive/v1` by package name, which only resolves
once the package is built and actually present in a `node_modules` next to
it. Drive it via
`.claude/skills/run-v1-playground/driver.mjs`, a Node REPL (Playwright +
...
**Do not confuse this with root-level `npm run playground`** — that serves
the *legacy* playground (`playground/index.html` / `playground/App.tsx`),
which is frozen and out of v1 scope. The v1 playground is a separate Vite
project at `playground/v1/`.
...
```bash
node .claude/skills/run-v1-playground/driver.mjs <<'EOF'
setup
serve
launch
section checkbox
...
per session, or again any time `src/v1` changes. `serve` and `launch` can be
...
Screenshots land in `/tmp/m3e-v1-playground-shots/` (override:
`SCREENSHOT_DIR`).

| command | what it does |
|---|---|
| `setup` | `npm run build`, pack, copy `playground/v1` into a scratch dir, link `node_modules` + `.bin`, extract the tarball into it |
...
| `section <name>` | screenshot just `.<name>-example` (the convention every `playground/v1/examples/<Name>.example.tsx` follows — `checkbox`, `card`, `button`, `icon-button`, `fab`, `icon`, `surface`, `text`) |
...
`playground/v1/examples/<Name>.example.tsx` with a `.<name>-example` root
class, wired into `playground/v1/src/main.tsx` — then `section <name>` works
...
# driver — there is no simpler manual dev-server path for playground/v1.
```
...
11 gates: typecheck, unit/interaction tests, package build, isolated v1
typecheck, architecture/browser-support/CSS/token/legacy-contract checks,
bundle-size budgets, and packed Vite/Next/legacy consumer fixture builds. This
...
- **`vite` must be launched with `cwd` set to the fixture directory**, not
  just pointed at it with `--config`. `playground/v1/vite.config.ts` sets no
...
- **`for await (const line of readlineInterface)` silently drops lines on
  piped/heredoc stdin.** This isn't a v1-specific bug — it's a genuine Node.js
...
- **`section <name>` reports `NOT_FOUND`**: either the component's example
  isn't wired into `playground/v1/src/main.tsx` yet, or its root class
  doesn't follow the `.<name>-example` convention every existing example
  uses (`grep -oE '\.[a-z-]+-example\b' playground/v1/src/playground.css` to
...
- **A stale fixture or vite process lingers after a crash**: the scratch
  directories live under `os.tmpdir()` as `m3e-v1-playground-*` — safe to
