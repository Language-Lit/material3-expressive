#!/usr/bin/env node
// REPL driver for the v1 playground (playground/v1). Drives a real Chromium
// page against the PACKED package build — the same thing a real consumer
// would import — not the raw source tree.
//
// Why this exists: playground/v1 imports `@language-lit/material3-expressive/v1`
// by package name, which only resolves once the package is built and that
// package is actually present in playground/v1's own node_modules. There is
...
//
// Usage: node .claude/skills/run-v1-playground/driver.mjs
...
const SHOT_DIR = process.env.SCREENSHOT_DIR || path.join(os.tmpdir(), 'm3e-v1-playground-shots')
...
const COMMANDS = {
  ...
  // fixture copied from playground/v1 — mirrors verify-consumer-fixtures.mjs.
  // Re-run this after any src/v1 change; it always starts from a clean copy.
  async setup() {
    console.log('building package (npm run build)...')
    run('npm', ['run', 'build'], ROOT)

    if (fixtureDir) rmSync(fixtureDir, { recursive: true, force: true })
    const scratchRoot = mkdtempSync(path.join(os.tmpdir(), 'm3e-v1-playground-'))
    fixtureDir = path.join(scratchRoot, 'playground-run')
    console.log('copying playground/v1 to', fixtureDir)
    run('cp', ['-r', path.join(ROOT, 'playground/v1'), fixtureDir])
  ...
  // playground/v1/examples/<Name>.example.tsx follows) instead of the whole
  ...
  async section(name) {
    ...
    try {
      ...
      return console.log('NOT_FOUND:', selector, '— is playground/v1/src/main.tsx wired to render it?')
    }
...
console.log('v1 playground driver — "help" for commands, "setup" to start')
safePrompt()
try {
